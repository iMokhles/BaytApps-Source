...
{
    [super viewDidLoad];
    if (self.allowCancelButton == YES) {
        self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(cancelEdit:)];
        
    } else {
        self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Home" style:UIBarButtonItemStyleDone target:self action:@selector(cancelEdit:)];
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCompose target:self action:@selector(customUrl:)];
...
- (void)customUrl:(id)sender {
    
    
    UIAlertController *alertController = [ITHelper showAlertViewForExtFromViewController:self WithTitle:@"Custom URL" msg:@"Enter your custom url" withActions:nil andTextField:YES];
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        
    }];
    UIAlertAction *goAction = [UIAlertAction actionWithTitle:@"Go" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        NSString *customUrl = alertController.textFields[0].text;
        if ([customUrl hasPrefix:@"http://"] || [customUrl hasPrefix:@"https://"]) {
           [self loadURL:[NSURL URLWithString:customUrl]];
        } else {
            [self loadURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://%@", customUrl]]];
        }
...
- (void)webView:(DZNWebView *)webView didStartProvisionalNavigation:(WKNavigation *)navigation
{
    ...
    NSString *str_url = webView.URL.absoluteString;
    if(navigation == WKNavigationTypeLinkActivated)
    {
        
    }
    if (([str_url containsString:@"filepup"] ||  [str_url containsString:@"sendspace"] || [str_url containsString:@"appd.be"] || [str_url containsString:@"mega.co"]  || [str_url containsString:@"mediafree"] || [str_url containsString:@"mediafire"] || [str_url containsString:@"cloudshares"] || [str_url containsString:@"ul.to"] || [str_url containsString:@"dailyuploads"] || [str_url containsString:@"userscloud"] || [str_url containsString:@"openload.co/stream"]) && ![str_url containsString:@"google"] ) {
        [_babrowserviewcontroller loadedLinkProcess:str_url];
        [self dismissViewControllerAnimated:YES completion:nil];
...
{
    //// NSLog(@"didUpdateProgress:: %@", webView.URL);
...
{
//    if ([webView.URL.absoluteString containsString:@"www.iplayplus.net"]) {
...
//        [webView loadRequest:navigationAction.request];
//    }
//    if ([webView.URL.absoluteString containsString:@"https://openload.co/stream"]) {
//
//        
//    }

    // NSLog(@"createWebViewWithConfiguration : %@",webView.URL);
    [webView loadRequest:navigationAction.request];
