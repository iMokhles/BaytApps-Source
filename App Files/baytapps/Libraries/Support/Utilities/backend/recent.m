	...
	[query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error)
	{
		if (error == nil)
		{
			if ([objects count] == 0)
			{
				...
				{
					if (error != nil) NSLog(@"CreateRecentItem save error. %@", error);
				}];
			}
		}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"NCCheckUnread" object:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"NCUpdateLeftTableChatCell" object:nil];
}
