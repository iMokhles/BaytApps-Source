...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
    ...
    isChatViewAppear = YES;
    AppDelegate *app = (AppDelegate *)[[UIApplication sharedApplication] delegate];
    app.isInChatView = YES;
}
	...
	[file getDataInBackgroundWithBlock:^(NSData *imageData, NSError *error)
	{
		if (error == nil)
		{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (imageData.length > 510) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        avatars[user.objectId] = [JSQMessagesAvatarImageFactory avatarImageWithImage:[UIImage imageWithData:imageData] diameter:30.0];
                        [self.collectionView reloadData];
                    });
                } else if (imageData.length <= 510) {
                    
                    UIImageView *profileImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 140, 140)];
                    [profileImageView setImageWithString:user[USER_FULLNAME] color:[UIColor whiteColor] circular:NO textAttributes:@{NSFontAttributeName: [profileImageView fontForFontName:nil],NSForegroundColorAttributeName: [BAColorsHelper sideMenuCellSelectedColors]}];
                    
                    dispatch_async(dispatch_get_main_queue(), ^{
                        avatars[user.objectId] = [JSQMessagesAvatarImageFactory avatarImageWithImage:profileImageView.image diameter:30.0];
                        [self.collectionView reloadData];
                    });
                }
            });
