...
//        CGFloat dy = ([UIDevice iosVersion]<7) ? 0 : MIN([UIApplication sharedApplication].statusBarFrame.size.height, [UIApplication sharedApplication].statusBarFrame.size.width);
        CGFloat yBar;
        if (self.isNavigationBarDown) {
            yBar = 50;
        } else {
            yBar = 20;
        }
        
        UINavigationBar *navigationBar = [[UINavigationBar alloc] initWithFrame:CGRectMake(0, yBar, self.view.width, 44)];
    ...
    [_navigationBar setTitleTextAttributes:@{NSForegroundColorAttributeName: [BAColorsHelper sideMenuCellSelectedColors]                                                           }];
    _navigationBar.tintColor = [BAColorsHelper sideMenuCellSelectedColors];
    _navigationBar.barTintColor = [BAColorsHelper ba_whiteColor];
//    _navigationBar.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"mainBG"]];
}
...
//        menuScroll.backgroundColor = [UIColor blackColor];
//        [menuScroll setOpaque:YES];
        [self.view addSubview:menuScroll];
        self.menuView = menuScroll;
    }
//    self.menuView.backgroundColor = [UIColor blackColor]; //[CLImageEditorTheme toolbarColor];
    self.menuView.backgroundColor = [BAColorsHelper sideMenuCellSelectedColors] ;//[UIColor colorWithPatternImage:[UIImage imageNamed:@"mainBG"]];
}
...
//            y = ([UIDevice iosVersion] < 7) ? y-[UIApplication sharedApplication].statusBarFrame.size.height : y;
    ...
    self.scrollView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"mainBG"]];
}
...
//                         CGFloat dy = ([UIDevice iosVersion]<7) ? [UIApplication sharedApplication].statusBarFrame.size.height : 0;
                         ...
                         if(size.width>0 && size.height>0){
                             ...
                             animateView.frame = CGRectMake((_scrollView.width-W)/2 + _scrollView.left, (_scrollView.height-H)/2 + _scrollView.top + 20, W, H);
                         }
