...
@property (nonatomic, copy) NSArray *filtereALogMessages;
...
{
    return self.searchController.isActive ? [self.filtereALogMessages count] : [self.logMessages count];
}
...
{
    return self.searchController.isActive ? self.filtereALogMessages[indexPath.row] : self.logMessages[indexPath.row];
}
...
- (void)updateSearchResultsForSearchController:(UISearchController *)searchController
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSArray *filtereALogMessages = [self.logMessages filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(FLEXSystemLogMessage *logMessage, NSDictionary *bindings) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([searchController.searchBar.text isEqual:searchString]) {
                self.filtereALogMessages = filtereALogMessages;
                [self.tableView reloadData];
            }
        });
    });
