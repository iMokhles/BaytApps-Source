...
@interface BAListSearchViewController () <UITableViewDelegate, UITableViewDataSource, LGRefreshViewDelegate, UISearchBarDelegate, UIGestureRecognizerDelegate> {
...
@implementation BAListSearchViewController
...
- (void)viewDidLoad {
    ...
    self.currentPage = 1;
    if (self.isLatestApps == NO && self.isRandomApps == NO && self.isMostPopularApps == NO && self.isCydiaApps == YES) {
...
- (void)loadApps {
    ...
    if (self.isLatestApps == NO && self.isRandomApps == NO && self.isMostPopularApps == NO && self.isCydiaApps == NO) {
        self.title = NSLocalizedString(@"Search", @"Search page title");
        ...
        self.title = NSLocalizedString(@"Latest Apps", @"Latest Apps page title");
        [[ITHelper sharedInstance] getAllAppsForCat:@"ios" page:self.currentPage withCompletion:^(NSArray *allApps, NSError *error) {
        ...
        self.title = NSLocalizedString(@"Most Popular", @"Most Popular page title");
        self.currentPage = 3;
        ...
        self.title = NSLocalizedString(@"Random Apps", @"Random Apps page title");
        ...
        self.title = NSLocalizedString(@"Cydia Apps", @"Cydia Apps page title");
...
- (void)refreshViewRefreshing:(LGRefreshView *)refreshView {
    ...
    self.currentPageSearch = self.currentPage+1;
    [self loadApps];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (isFiltered) {
        ...
        [[ITHelper sharedInstance] getAppInfoFromItunes:app.appInfo[@"id"] withCompletion:^(NSArray *allApps, NSError *error) {
            ...
            appDescrip.isCydiaApp = self.isCydiaApps;
            @try {
...
//                        self.isRefreshing = NO;
            ...
            isFiltered = YES;
            self.isRefreshing = YES;
            [[ITHelper sharedInstance] searchWithKeyword:self.searchBar.text page:self.currentPageSearch withCompletion:^(NSArray *allApps, NSError *error) {
